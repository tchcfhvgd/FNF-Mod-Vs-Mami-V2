...
class PlayState extends MusicBeatState
{
	...
	private function generateSong(dataPath:String):Void
	{
		...
		if (SONG.needsVoices)
			{
				if (ClientPrefs.useSFBF) vocals = new FlxSound().loadEmbedded(Paths.voicesSF(PlayState.SONG.song));
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Camera Zoom Freq/Depth':
				...
				if(Math.isNaN(camZoomFreq) || camZoomFreq == 0) camZoomFreq = 4;
				if(Math.isNaN(camZoomDepth)) camZoomDepth = 1.0;
	
			case 'Health Drain Note':
				healthDrainNoteAmt = Std.parseFloat(value1);
				healthDrainLowestPoint = Std.parseFloat(value2);

				if(Math.isNaN(healthDrainNoteAmt)) healthDrainNoteAmt = 0.0;
				if(Math.isNaN(healthDrainLowestPoint)) healthDrainLowestPoint = 0.05;
		}
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		switch (daNote.noteType)
		{
			case "Holy Note":
				trace(holyNoteDmg);
				if (!daNote.isSustainNote) FlxG.sound.play(Paths.sound('MAMI_shoot', 'shared'));
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if (healthDrainNoteAmt != 0.0 && health > healthDrainLowestPoint && !note.isSustainNote) healthChange(healthDrainNoteAmt * -1);
		if (healthDrainNoteAmt != 0.0 && health > healthDrainLowestPoint && note.isSustainNote) healthChange(healthDrainNoteAmt * -0.33);
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if (note.noteType == "Holy Note")
				{
					if (!note.isSustainNote) FlxG.sound.play(Paths.sound('MAMI_shoot', 'shared'));
	...
	override function beatHit()
	{
		...
		{
			boyfriend.dance();
			if (canCameraMove && SONG.notes[Math.floor(curStep / 16)].mustHitSection) isCameraOnForcedPos = false;
		}
